...
public class Queries {
	...
	private static void consultaD(Session session, Date inicio, Date fin){
		Transaction tx = null;
		System.out.println("d. Listar moderadores que hayan revisado alguna traducción entre dos fechas pasadas como argumento.");
		Query query = session.createQuery("select distinct moderador from Moderador moderador join moderador.evaluaciones evaluaciones where evaluaciones.fecha between :inicio and :fin")
	...
	private static void consultaF(Session session){
		...
		Query query = session.createQuery("select distinct usuario "
				+ "from Usuario as usuario "
				+ "join usuario.cursadasRealizadas as cr "
				+ "where cr not in "
					+ "(select cursada from Cursada as cursada "
					+ "join cursada.curso.lecciones as lec "
					+ "join cursada.pruebas as pru "
					+ "where lec not in"
						+ "(select leccion from Usuario as user "
						+ "join user.cursadasRealizadas as cursadasR "
						+ "join cursadasR.pruebas as pruebas "
						+ "join pruebas.leccion as leccion "
						+ "where pruebas.puntaje >= 60 and user = usuario "
						+ "group by leccion))");
